		...
		<div class="terms_ct">
			...
			<div class="btn_type_03">
				...
				<c:if test="${session_member_grade == '1'}">
					<a href="#none" onclick="deleteAdminQna()" class="btn btnC_03 btnP_04 mr5"> <span>삭제</span>
					</a>
				</c:if>
				<c:if test="${session_member_grade != '1'}">
				...
				<c:if test="${session_member_grade == '1'}">
					<%-- <a href="<%=cp%>/qna/qnaReply.mt?qna_no=${qnaModel.qna_no}&currentPage=${currentPage}" class="btn btnC_03 btnP_04"> --%>
						<a href="<%=cp%>/admin/adminQnaReply.mt?qna_no=${qnaModel.qna_no}&currentPage=${currentPage}" class="btn btnC_03 btnP_04">
						<span>답변</span>
					</a>
				</c:if>
