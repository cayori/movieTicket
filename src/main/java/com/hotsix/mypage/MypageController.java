...
public class MypageController {
	...
	public ModelAndView mcoinPageSuccess(@ModelAttribute("member") MemberModel member, BindingResult result, 
		...
		System.out.println(memberModel.getMticket_coin() + "총금액");
		System.out.println(memberModel.getAdult_ticket() + "성인티켓갯수");
		System.out.println(memberModel.getChild_ticket() + "어린이티켓갯수");
		// 코인부족
		if(mticket_coin > memberModel.getMticket_coin()){
			mav.setViewName("mypage/mticketNO");
			
			System.out.println(memberModel.getMticket_coin() + "코인부족");
			return mav;
		// 티켓 갯수 선택 안하고 교환 버튼 눌렀을때
		} else if(adult_ticket == 0 && child_ticket == 0) {
			mav.setViewName("mypage/mticketZero");
			
			System.out.println(memberModel.getAdult_ticket() + "성인티켓");
			return mav;
		// 성공했을때
		} else {
			System.out.println("성공했을 때");
